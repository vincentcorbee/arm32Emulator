...
export const symbol = map(
  identifier,
  value => ({ type: 'Label', name: value })
)
export const decimal = map(
  mapJoin(
    many(decimalDigit)
  ),
...
export const binary = map(
  chain(
    binaryPrefix,
    () => mapJoin(many(binaryDigit))
  ),
  value => parseInt(value, 2)
)
export const characterConstant = map(
  sequence(
    singleQuote,
    regexp(/[^']/),
    singleQuote
  ),
  value => ({ type: 'CharacterConstant', value: value[1].charCodeAt(0) })
)
export const stringConstant = map(
  sequence(
    doubleQuote,
    regexp(/[^"]*/),
    doubleQuote
  ),
  value => ({ type: 'StringConstant', value: value[1] })
)