...
const list: any = lazy(() =>
  map(or(sequence(expression, optionalWhitespace, comma, optionalWhitespace, list), expression), (value) => {
...
export const wordDirective = map(sequence(directiveName('word'), many(whitespace), list), (value) => ({
  type: 'Directive',
  name: value[0].value,
  value: value[2].value,
}));
